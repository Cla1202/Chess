...
public class ProfileFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        // Inizializza ViewModel per le statistiche
    ...
    private void calculateAndDisplayStats(List<LevelProgress> list, TextView totalTv, TextView accTv, TextView streakTv, TextView timeTv, TextView levelTv) {
        ...
        // Precisione: (Totale - Errori) / Totale (approssimativa per esercizio)
        // Oppure basata sul numero di quiz fatti senza errori al primo colpo
        int perfectSolved = 0;
        long totalTime = 0;
        ...
        // Calcolo Serie (Streak)
        int currentStreak = 0;
        int maxStreak = 0;
        // Supponiamo che la lista sia ordinata per timestamp
        for (LevelProgress p : list) {
            if (p.mistakesMade == 0) {
                currentStreak++;
        ...
        streakTv.setText(maxStreak + " \uD83D\uDD25");

        // Titolo in base al numero di quiz
        if (total < 5) levelTv.setText("Novizio");
        else if (total < 10) levelTv.setText("Apprendista");
        else if (total < 15) levelTv.setText("Esperto");
        else levelTv.setText("Gran Maestro");
    }
